    ...
    fetch('https://www.overpass-api.de/api/interpreter?data=[out:json];node(id:' + osmIds + ');out;')
        ...
        .then(function(jsonResponse) {
            for (let j = 0; j < jsonResponse.elements.length; j++) {
                ...
                const cafeHours = (jsonResponse.elements[j].hasOwnProperty("tags") === false || jsonResponse.elements[j].tags.hasOwnProperty("opening_hours") === false) ? "<a href=\"https://www.openstreetmap.org/edit?node=" + cafes[j].osm : jsonResponse.elements[j].tags.opening_hours;
